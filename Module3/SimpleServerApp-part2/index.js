...
const fs        = require('fs'); //on appel le package file system pour enregistrer l'image sur le disque
...
app.post('/api/send', function(request, response){
    ...
    let pixelData       = img64.replace(/^data:image\/\w+;base64,/, ""); //on supprime l'entête de la data pour ne récupérer que les données
    let filename        = imgFolder+"seflie_"+timestamp+extension;
    fs.writeFile("public/"+filename, pixelData, 'base64', function(error){

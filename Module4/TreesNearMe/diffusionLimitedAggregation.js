...
function preload(){
    if("geolocation" in navigator){
        navigator.geolocation.getCurrentPosition(async function(position){
            ...
            let spanarbre           = document.getElementById("arbres");
            let spankm              = document.getElementById("km");
            spanarbre.textContent   = jsondata.nhits;
            spankm.textContent      = dist/1000.0;

            let nbParticles = round(jsondata.nhits * .1);
            let it = 3;
            
            for(let j=0; j<it; j++){
                let nb = nbParticles/it;
                let radInc = map(j, 0, it-1, 0.1, .45)
                let randAngle = random(TWO_PI);
                for(let i=0; i<nb; i++){
                    let angle   = (TWO_PI / nb) * i + randAngle;
                    let rad     = height * radInc;
            ...
            for(let i=0; i<jsondata.nhits - nbParticles; i++){
                ...
                let rad     = random(height * .1, height * .4)
                let x       = cos(angle) * rad + width/2;
                let y       = sin(angle) * rad + height/2; 
...
function draw(){
    ...
    for(let j=0; j<walkers.length; j++){
        ...
        walker.x += random(-1, 1) * 10;
        walker.y += random(-1, 1) * 10;
